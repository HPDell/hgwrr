...
The following is explaintion of some important parameters.
...
data(wuhan.hp)
m_sf <- hgwr(
  formula = Price ~ L(d.PrimarySchool + d.ShoppingMall) + BuildingArea + (Floor.High | group),
  data = wuhan.hp
)
```
...
- Article [*HGWR Model and How to Use It*](`r paste0(strsplit(packageDescription("hgwrr")[["URL"]], ",\\n")[[1]][2], "articles/introduction.html")`) in this site compares HGWR with GWR and HLM on the simulation data.
