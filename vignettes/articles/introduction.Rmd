...
```bash
R CMD INSTALL hgwrr_0.5-0.tar.gz
```
...
- `formula` accepts a formula object in R. Its format follows **lme4** package.
  As there are three types of effects: GLSW effects, fixed effects and SLR effects,
  we use the following format to specify both of them:

    ```r
    dependent ~ L(glsw) + fixed + (slr | group)
    ```
...
In the former example, there are only `r nrow(mulsam.test$data)` observations and `r nrow(mulsam.test$coords)` groups.
...
Its structure is similar to data `mulsam.test`.
...
> As the data is large (`r nrow(multisampling$data)` observations),
